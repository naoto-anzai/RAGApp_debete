...
from langchain_groq import ChatGroq       # 実習と同じGroqを使う

from debate_agent.debate_graph import create_debate_graph
from debate_agent.debate_state import DebateState

load_dotenv()                             # .env の GROQ_API_KEY を読み込む
...
Codex 等のコード生成エージェントに渡す **実装手順書** は
[08_実装計画_Codex.md](08_実装計画_Codex.md)。
確定スタック(Groq固定)・実装順序・ステップごとの動作確認・そのまま渡せる指示文まで含む。
